...
const TrailCard = React.forwardRef<HTMLDivElement, TrailCardProps>(({ className, imageUrl, title, location, onClick }, ref) => {
	return (
		<motion.div
			ref={ref}
			className={cn('w-full cursor-pointer overflow-hidden rounded-2xl bg-card text-card-foreground shadow-lg', className)}
		...
		>
			<div className='relative h-60 w-full'>
				...
				<div className='absolute inset-0 z-0 bg-linear-to-t from-black/70 via-black/30 to-transparent' />

				{/* RIGHT-SIDE BLUR overlay (only this side will blur the image behind it) */}
				<div
					aria-hidden
					className='pointer-events-none absolute bottom-0 z-10 w-24 bg-white/6 backdrop-blur-md'
					style={{
				...
				{/* content above blur */}
				<div className='absolute bottom-0 left-0 z-20 flex w-full items-end justify-between p-4'>
